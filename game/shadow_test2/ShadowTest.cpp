...
void ShadowTest::addObject(Entity& root, const std::string& model, const Material& material, const glm::vec3& position, const glm::vec3& scale) {
...
ShadowTest::ShadowTest() {
	...
	spotLight->addComponent(new MovableComponent());
	m_gameWorld.rootEntity.addChildEntity(spotLight);

	addObject(m_gameWorld.rootEntity, "cube.obj", bricks, glm::vec3(ROOM_WIDTH / 4.0f, ROOM_HEIGHT / 2.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));

	addObject(m_gameWorld.rootEntity, "monkey3.obj", bricks, glm::vec3(-ROOM_WIDTH / 4.0f, ROOM_HEIGHT / 2.0f, 0.0f), glm::vec3(0.5f, 0.5f, 0.5f));
